...
const NavBar = ({theme,setTheme}) => {
    return (
        <>
         <div data-aos="zoom-in-up"  className=' fixed right-5 hidden  bg-accent z-0  h-screen top-0 items-center justify-center md:flex  flex-col    text-neutral text-5xl  '>        
        ...
        {/* mobile device */}
         ...
         <button onClick={()=>setTheme(!theme)}><FaLightbulb className=' p-3  bg-primary hover:duration-700 hover:bg-info rounded-full shadow-md '/></button>
          ...
          <NavLink to='/contact' ><FaPhone className='p-3 bg-primary hover:duration-700 hover:bg-info rounded-full shadow-md'/></NavLink>
          <NavLink to='/blog' ><FaBlogger className='p-3  ml-5 my-5 bg-primary hover:duration-700 hover:bg-info rounded-full shadow-md'/></NavLink>         
        </div>
        </>
    );
};
